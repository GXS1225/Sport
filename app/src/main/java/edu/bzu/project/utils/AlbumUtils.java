...
public class AlbumUtils {
	...
	HashMap<String, ImageBucket> bucketList = new HashMap<String, ImageBucket>();//记录图片文件夹
	 ...
	 * @return
	 */
	public static AlbumUtils getHelper() {
		if (instance == null) {
	...
	void buildImagesBucketList() {
		...
		if (cur.moveToFirst()) {
			...
			do {
				...
				ImageBucket bucket = bucketList.get(bucketId);//读取图片所在的文件夹的id，防止重复的读取同一个文件
				if (bucket == null) {
 ...
 */
	public List<ImageBucket> getImagesBucketList(boolean refresh) {
		if (refresh || (!refresh && !hasBuildImagesBucketList)) {
			buildImagesBucketList();//建立图片文件夹的集合，
		}
