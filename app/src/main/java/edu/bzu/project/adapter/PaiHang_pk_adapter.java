...
import edu.bzu.project.adapter.PaiHang_adapter.viewHolder;
...
public class PaiHang_pk_adapter extends BaseAdapter {

	private Context context =null;
	private List<PaiHang_domain> lists =null;
	
	public PaiHang_pk_adapter(Context context, List<PaiHang_domain> lists) {
		super();
	...
	public View getView(final int position, View convertView, ViewGroup parent) {
		...
		v.juli.setText("Ó®"+(int)lists.get(position).getSumScore()+"¾Ö");
		v.paihang.setText("µÚ"+(position+1)+"Ãû");
		return convertView;
	}
	class viewHolder{
		ImageView icon;
